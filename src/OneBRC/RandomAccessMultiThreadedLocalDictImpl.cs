...
public class RandomAccessMultiThreadedLocalDictImpl
{
    private readonly string _filePath;

    public RandomAccessMultiThreadedLocalDictImpl(string filePath)
    {
        _filePath = filePath;
    }
    ...
    public ValueTask<Dictionary<string, Accumulator>> Run()
    {
        ...
        var aggregates = new Dictionary<Key, Accumulator>[Environment.ProcessorCount];
        ...
        Parallel.For(0, offsets.Length, options, i =>
        {
            ...
            {
                ProcessChunk(fileHandle, offsets[i], offsets[i + 1] - offsets[i], aggregates[i]);
            }
            else
            {
                ProcessChunk(fileHandle, offsets[i], lastLength, aggregates[i]);
            }
        });

        var all = new Dictionary<Key, Accumulator>();

        foreach (var aggregate in aggregates)
        {
            foreach (var (key, value) in aggregate)
            {
                if (!all.TryGetValue(key, out var accumulator))
                {
                    all[key] = value;
                }
                else
        ...
        var accumulators = all.Values.ToDictionary(v => v.City!);
    ...
    private static void ProcessChunk(SafeFileHandle fileHandle, long offset, long length,
        Dictionary<Key, Accumulator> aggregate)
    {
